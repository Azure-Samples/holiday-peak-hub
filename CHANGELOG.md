...
- Foundry V3 Hosted Agents pilot end-to-end scaffolding (issue #990 follow-up). New library module `lib/src/holiday_peak_lib/foundry_hosting/` ships a Pydantic `HostedAgentManifest` schema (validates `template.kind: hosted` + `protocols`/`environment_variables`/`resources` per the canonical `agent.manifest.yaml` shape used by `azd ai agent`) and a thin `deploy_hosted_agent_version()` wrapper around `AIProjectClient.agents.create_version(... HostedAgentDefinition(container_protocol_versions=[ProtocolVersionRecord(...)], cpu, memory, image, environment_variables))` with optional polling until the version reports a terminal status. CLI entry point at `scripts/ops/deploy_hosted_agent.py` consumes the validated manifest and reports JSON-friendly results for CI. Pilot service `apps/inventory-health-check` adds a sibling manifest `agent.hosted.yaml` (the existing `agent.yaml` remains the fleet-wide portal-tracking artifact governed by `tests/ops/test_foundry_portal_tracking_manifests.py`) with `kind: hosted`, `responses 1.0.0` protocol, `gpt-5-nano` / `gpt-5` model resources, and `UVICORN_PORT=8088` baked into `template.environment_variables`. Dockerfile final CMD now honours `${UVICORN_PORT:-8000}` so the same image runs on AKS (default port 8000) and Foundry-hosted (port 8088). Mount default in `holiday_peak_lib.agents.hosted.mount_hosted_agent` corrected from `prefix="/v1"` to `prefix=""` so the container serves `/responses` directly — the Foundry gateway adds the public `/openai/v1/` segment before routing to the container. Full V3 deploy runbook: build the image with `--platform linux/amd64`, push to the configured ACR, then run `python scripts/ops/deploy_hosted_agent.py --agent-yaml apps/inventory-health-check/agent.hosted.yaml --image-uri <acr>/inventory-health-check:<tag> --project-endpoint $PROJECT_ENDPOINT`.
...
- Foundry V3 hosted-agents portal-visibility blockers (PR #1103 follow-up after the deployed pilot did not materialize in the New Foundry portal). Four issues identified by line-by-line comparison against the canonical `Microsoft/foundry-samples` repository and the official "Deploy a hosted agent" Learn doc: (1) `AIProjectClient` must be instantiated with `allow_preview=True` to expose the `agents.create_version` V3 surface — without the flag the SDK silently falls back to the legacy assistants surface and the new agent never appears. Fixed in `lib/src/holiday_peak_lib/foundry_hosting/deploy.py::_build_project_client`. (2) Terminal status set was over-broad (`{active, ready, succeeded, failed, error}`) and would mark a still-`creating` deployment as terminal on the first poll if the API ever returned `succeeded`. Tightened to the documented terminal set `{active, failed, deleted}` with `active` as the only success terminal; `deleting` is now correctly treated as transient. (3) `apps/inventory-health-check/agent.hosted.yaml` redeclared `APPLICATIONINSIGHTS_CONNECTION_STRING`, which collides with the Foundry-platform-injected variable of the same name (along with `FOUNDRY_PROJECT_ENDPOINT`, `FOUNDRY_PROJECT_ARM_ID`, `FOUNDRY_AGENT_NAME`, `FOUNDRY_AGENT_VERSION`, `FOUNDRY_AGENT_SESSION_ID`) — collisions cause `create_version` to reject the manifest. Pruned and documented the forbidden list inline. (4) Pilot manifest declared `FOUNDRY_AGENT_NAME_FAST` / `_RICH`, but the runtime contract per ADR-010 / `holiday_peak_lib.config._build_foundry_config("FOUNDRY_AGENT_ID_FAST", ...)` reads `FOUNDRY_AGENT_ID_FAST` / `_RICH`. Renamed in the manifest. Loader (`load_manifest`) now also probes `agent.manifest.yaml` (the canonical Microsoft `foundry-samples` filename used by `azd ai agent init -m`) before falling back to `agent.hosted.yaml` and `agent.yaml`, so future services may adopt either name.

- Foundry V3 hosted-agents — env-var namespace reservation (PR #1103, discovered live against the platform after the four fixes above). The previous rename to `FOUNDRY_AGENT_ID_FAST` / `_RICH` was still rejected by `AIProjectClient.agents.create_version` with `ValidationError: Environment variable 'FOUNDRY_AGENT_ID_FAST' is reserved for platform use. All FOUNDRY_* and AGENT_* variables are reserved per container-image-spec`. The platform reserves both the `FOUNDRY_*` **and** `AGENT_*` env-var namespaces in their entirety, not just the six documented platform-injected names. Fix: the pilot manifest now maps the logical SLM/LLM agent IDs through the non-reserved `HPH_AGENT_ID_FAST` / `HPH_AGENT_ID_RICH` env vars (and matching `HPH_AGENT_NAME_*` for the metadata-only path). `holiday_peak_lib.app_factory_components.foundry_lifecycle.build_foundry_config` now reads the `HPH_` prefix first and falls back to the legacy `FOUNDRY_AGENT_ID_*` / `FOUNDRY_AGENT_NAME_*` for AKS deploys that have not migrated, so the change is fully back-compat with the AKS env-var contract. The operator still passes values via `$env:FOUNDRY_AGENT_ID_FAST` at registration time — the manifest's placeholder substitution wires them into the container as `HPH_AGENT_ID_FAST`.

- Foundry V3 hosted-agents — status normalization for `agentversionstatus.failed` (PR #1103). The Azure AI Projects SDK 2.1.0 deserialises the JSON `"status": "failed"` field into an `AgentVersionStatus` enum whose `str()` representation is `"AgentVersionStatus.FAILED"`. The previous `str(status).lower()` pipeline produced `"agentversionstatus.failed"`, which never matched the terminal sets (`_TERMINAL_STATUSES`, `_FAILED_STATUSES`) so the poll loop spun until the timeout. Added `_normalize_status` in `lib/src/holiday_peak_lib/foundry_hosting/deploy.py` that prefers the Enum's `.value` field and falls back to stripping any `Enum.MEMBER` dotted prefix so unexpected SDK shapes still normalise to canonical lowercase status strings. Three new tests in `lib/tests/test_foundry_hosting_deploy.py` cover the Enum-value path, the dotted-string fallback, and plain string inputs.
