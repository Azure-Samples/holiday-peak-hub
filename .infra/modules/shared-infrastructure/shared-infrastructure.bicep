...
@secure()
@description('Seed used to derive the autogenerated PostgreSQL password when postgresAdminPassword is empty. Defaults to a deterministic value derived from subscription + resource group + server name so repeat deploys do NOT rotate the password. Override only to force an intentional rotation.')
param postgresAdminPasswordSeed string = uniqueString(subscription().id, resourceGroup().id, 'postgres-admin-v1')
...
module keyVault 'br/public:avm/res/key-vault/vault:0.13.3' = {
  ...
  params: {
    ...
    roleAssignments: [
      ...
      {
        // CRUD workload identity -> Key Vault (Secrets Officer)
        // Officer grants read + write + list so the CRUD service can self-heal
        // by reconciling its PostgreSQL credential against Key Vault when it
        // detects drift. Read-only Secrets User was insufficient for automated
        // recovery paths.
...
// Permanent control-plane grant: CRUD workload identity -> PostgreSQL Flexible
// Server (Contributor). Enables the CRUD service to update server-level
// configuration (for example, administratorLoginPassword rotation during
// automated credential reconciliation) without requiring a human Owner to
// re-grant permissions after every provision. Standalone (not inline) because
// the AVM module does not expose role assignments at the time of this change.
resource crudPostgresContributor 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(resourceGroup().id, 'crudWorkloadIdentity', 'postgres-contributor', postgresServerName)
