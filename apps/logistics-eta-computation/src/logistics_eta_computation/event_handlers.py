"""Event handlers for logistics ETA computation service."""
...
from .adapters import build_eta_adapters


def build_event_handlers() -> dict[str, EventHandler]:
    """Build event handlers for ETA computation subscriptions."""
    logger = configure_logging(app_name="logistics-eta-computation-events")
    adapters = build_eta_adapters()
    ...
    async def handle_order_event(partition_context, event) -> None:  # noqa: ANN001
        ...
        if not tracking_id:
            logger.info("eta_event_skipped", event_type=payload.get("event_type"))
            return
