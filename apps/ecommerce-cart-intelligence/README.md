...
Analyzes shopping carts to identify friction, abandonment risk, and conversion opportunities. Scores cart-level risk signals and recommends actions to improve checkout completion.
...
| Tool | Description |
|------|-------------|
| `/cart/context` | Build full cart context with product, pricing, and inventory |
| `/cart/abandonment-risk` | Estimate cart abandonment risk score |

### Event Subscriptions
| Topic | Consumer Group | Action |
|-------|---------------|--------|
| `order-events` | `cart-intel-group` | React to order patterns for cart scoring |
...
| Tier | Purpose |
|------|---------||
| Hot (Redis) | Cached cart analysis results (TTL 300s) |
| Warm (Cosmos DB) | Cart session state |
| Cold (Blob) | Historical cart abandonment data |
...
uv sync
uv run uvicorn ecommerce_cart_intelligence.main:app --reload --port 8005
```

## Test Coverage
```bash
python -m pytest apps/ecommerce-cart-intelligence/tests
```
