...
from .adapters import build_carrier_selection_adapters


def build_event_handlers() -> dict[str, EventHandler]:
    """Build event handlers for carrier selection subscriptions."""
    logger = configure_logging(app_name="logistics-carrier-selection-events")
    adapters = build_carrier_selection_adapters()
    ...
    async def handle_order_event(partition_context, event) -> None:  # noqa: ANN001
        ...
        if context is None:
            logger.info(
                "carrier_event_missing",
                event_type=payload.get("event_type"),
                tracking_id=tracking_id,
            )
            return

        selection = await adapters.selector.select(context)
        logger.info(
            "carrier_event_processed",
            event_type=payload.get("event_type"),
            tracking_id=tracking_id,
            recommended_carrier=selection.get("recommended_carrier"),
            service_level=selection.get("service_level"),
        )

    return {
        "order-events": handle_order_event,
        "shipment-events": handle_order_event,
    }
