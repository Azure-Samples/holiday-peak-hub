...
## Domain Scope
Cover schema-based gap detection across all 18 UCP retail categories (apparel, footwear, electronics, home furniture, beauty, toys & games, sports & outdoors, books & media, jewelry & watches, food & gourmet, pet supplies, automotive, garden & outdoor, office supplies, health & wellness, baby & kids, home appliances, musical instruments). Perform per-field AI enrichment (text and image analysis), confidence scoring, candidate merging, and HITL routing. You read products and schemas from Blob Storage, invoke Foundry models for attribute generation, and publish enrichment proposals to the HITL review queue. Resolve category aliases automatically — the schema registry maps 40+ alias strings to canonical category IDs. Do not approve or reject proposals — that is the HITL agent's responsibility.
