...
| App Factory | `lib/src/holiday_peak_lib/app_factory.py` | Standard service bootstrapping | Factory Pattern |
| Config | `lib/src/holiday_peak_lib/config/` | MemorySettings, ServiceSettings, TruthLayerSettings | Configuration |
| Events | `lib/src/holiday_peak_lib/events/` | RetailEvent, ConnectorEvent models, schema versioning | Event-Carried State Transfer |
| MCP | `lib/src/holiday_peak_lib/mcp/` | FastAPIMCPServer for agent-to-agent tools | MCP Protocol |
| Self-Healing | `lib/src/holiday_peak_lib/self_healing/` | SelfHealingKernel with incident lifecycle | Observer Pattern |
| Truth | `lib/src/holiday_peak_lib/truth/` | Product truth layer schemas and utilities | Domain Model |
| Evaluation | `lib/src/holiday_peak_lib/evaluation/` | Agent response evaluation framework | Strategy Pattern |
...
| Messaging | `lib/src/holiday_peak_lib/messaging/` | Event Hub publisher/subscriber wiring | Messaging Pattern |
