...
- **Single-architecture guardrail (from the inventory hosted-agent lesson):** A service must not ship a second entry point (e.g., `hosted_main.py`, secondary port, or a second FastAPI/Starlette runtime) alongside `main.py`. The MAF `Agent` is constructed inside the existing FastAPI handler in `main.py`. An AKS-hosted Responses adapter is allowed only when it is mounted into that same FastAPI app and same pod/port as `/health`, `/ready`, `/mcp/*`, and `/invoke`. A Foundry-managed hosted container registered via `AIProjectClient.agents.create_version` with `template.kind: hosted` is not the product path for `inventory-health-check`. PRs introducing parallel runtimes or Foundry-managed hosted-container product manifests are rejected at review.
