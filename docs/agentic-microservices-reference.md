...
| **Microsoft Agent Framework (MAF)** | `agent-framework>=1.2.0` runs `agent_framework.Agent` in-process over a pluggable `ChatClient` (default `agent_framework_foundry.FoundryChatClient`). The legacy `FoundryAgentInvoker` portal-agent path is transitional pending Wave 4 cleanup. |
