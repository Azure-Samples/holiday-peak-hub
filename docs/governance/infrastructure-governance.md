...
- Main-branch changes to `azure.yaml`, `.infra/**`, `.kubernetes/**`, and deploy workflow files must trigger `test` so the canonical `deploy-azd-dev.yml` `workflow_run` promotion path remains the automatic deployment fan-in for infrastructure and deployment-orchestration changes.
- Main-branch `workflow_run` deploys must diff the tested source commit against its first parent, and the reusable deploy workflow must fetch enough default-branch history for that parent to be available before falling back to a default-branch baseline.
- UI-only Static Web Apps deployments that use Next.js `output: "standalone"` must copy `.next/static` and `public` into `.next/standalone` before upload so SWA serves the app shell instead of a platform 404.
- Next.js hybrid Static Web Apps deployments must not define `navigationFallback` in `staticwebapp.config.json`; fallback routing to `/index.html` is for static exports and can mask the hybrid server-rendered app with a platform 404.
...
- Agent-service deployments must enforce the direct-model runtime contract end to end: workflow intent must preserve `PROJECT_ENDPOINT` / `FOUNDRY_ENDPOINT` and `MODEL_DEPLOYMENT_NAME_FAST` / `MODEL_DEPLOYMENT_NAME_RICH`, render hooks must emit those keys into Helm output, and the post-deploy gate must compare workflow intent, rendered manifests, live Deployment env values, and live `/ready` behavior for each changed agent service.
- A changed agent service is a hard deployment failure when any direct-model contract seam drifts: missing rendered keys, rendered-versus-live env mismatch, missing model deployment names, or `/ready` responses that remain healthy while configured `maf-direct` targets are not actually bound.
