...
- **MAF Direct-Model Invocation is now the canonical agent runtime** (Issue #990, ADR-005 amendment 2026-05-10): all 26 agent services construct an in-process `agent_framework.Agent` over a pluggable `ChatClient` (`FoundryChatClient` by default). Foundry remains the model-deployment plane, the OTel telemetry backend, and the Evaluations API surface - not an agent-runtime intermediation layer. New seam: `DirectModelInvoker` (lib) + `AgentBuilder.with_direct_models()` + `use_direct_model=True` kwarg / `HOLIDAY_PEAK_DIRECT_MODEL=true` env on `build_service_app` / `create_standard_app`. Wave 4c removed the retired portal-agent runtime, `/foundry/agents/ensure`, and the V2 provisioning code path from the framework. The 42 portal-managed agents in project `aipholidaris` are intentionally left untouched for manual deprovisioning.
...
- `skipProvision=true` is a manual dev-only emergency path for already-provisioned infrastructure. It skips only the `azd provision` step; Azure login, azd environment setup, AKS/ACR/Key Vault guards, output export, image build, deploy, direct-model readiness, and downstream smoke/deploy gates remain active.
...
- For changed AKS agent services, treat the direct-model runtime contract as a blocking gate: `PROJECT_ENDPOINT` / `FOUNDRY_ENDPOINT` and `MODEL_DEPLOYMENT_NAME_FAST` / `MODEL_DEPLOYMENT_NAME_RICH` must survive render and rollout, and `/ready` is only accepted when configured `maf-direct` targets are bound.
...
12. **Postdeploy direct-model readiness (`azd deploy` / `azd up`)**

- The retired `ensure-foundry-agents` postdeploy hook and `/foundry/agents/ensure` endpoint were removed in Wave 4. Agent readiness is now validated through `/ready` against direct-model target binding only.
