...
def test_drift_detector_emits_report_after_configured_failure_window() -> None:
    ...
    baseline = EvalBaseline(agent_name="catalog-search", metrics={"relevance": 0.9})
...
def test_agent_builder_wires_evaluation_config() -> None:
    config = EvalConfig(agent_name="catalog-search")

    agent = AgentBuilder().with_agent(_Agent).with_evaluation(config).build()

    assert agent.evaluation_config == config
...
def test_evaluation_run_and_history_endpoints(tmp_path: Path) -> None:
    ...
    register_standard_endpoints(
        ...
        ensure_agents_handler=lambda _payload: _async_payload({"foundry_ready": True}),
    ...
    assert run_response.json()["result"]["backend"] == "local-fallback"
